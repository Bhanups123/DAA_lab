...
int num_Phil = 5; // Number of philosphers
thread philosphers[num_Phil]; // The Philosophers as threads
mutex mtx[num_Phil]; //forks as mutexes
mutex cout_mutex; // mutex for keeping cout safe
...
void print(string str){ // Function for safe printing
...
void think(int id){ // Philosopher "id" is thinking
    this_thread::sleep_for(chrono::milliseconds(500)); // thinking - time : 500 ms
...
void dine(){
    ...
    for (int i = 0; i < num_Phil; ++i) philosphers[i] = thread(dinner_started, i);
    // Threads joined...
...
int main() {
    ...
    for (int i = 0; i < num_Phil; ++i) cout << i << " = " << ate[i] << endl;
}
